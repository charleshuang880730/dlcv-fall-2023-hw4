...
from grade import rgb_lpips, rgb_ssim
...
if __name__ == "__main__":
    ...
    dir_name = f'{args.scene_name}'
    # os.makedirs(dir_name, exist_ok=True)

    for i in tqdm(range(len(dataset))):
        sample = dataset[i]
        image_id = dataset.split_ids[i]
        ...
        if args.save_depth:
            ...
            if args.depth_format == 'pfm':
                ...
                # cv2.imwrite(os.path.join(dir_name, f'{image_id:05d}_depth.png'), depth_color)
            else:
        ...
        imgs += [img_pred_]
        # imageio.imwrite(os.path.join(dir_name, f'{i:05d}.png'), img_pred_)
        imageio.imwrite(os.path.join(dir_name, f'{image_id:05d}.png'), img_pred_)
        ...
        if 'rgbs' in sample:
            ...
            ssim_val = rgb_ssim(img_gt, img_pred, max_val=1)
            ssim.append(ssim_val)

            img_gt_np = img_gt.cpu().numpy()
            lpips_val = rgb_lpips(img_gt_np, img_pred, net_name='alex', device='cuda')
    ...
    if psnrs:
        ...
        print(f"Mean LPIPS : {mean_lpips:.2f}")