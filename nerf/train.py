...
class NeRFSystem(LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if batch_nb == 0:
            ...
            # 使用 wandb.log 記錄圖像
            if self.logger:
                self.logger.experiment.log({"val/GT_pred_depth": [wandb.Image(img) for img in stack]})


        log['val_psnr'] = psnr(results[f'rgb_{typ}'], rgbs)
        self.log('val_loss', log['val_loss'], on_epoch=True, prog_bar=True, logger=True)
        self.log('val_psnr', log['val_psnr'], on_epoch=True, prog_bar=True, logger=True)
        return log
        # return {'val_loss': log['val_loss'], 'val_psnr': log['val_psnr']}
